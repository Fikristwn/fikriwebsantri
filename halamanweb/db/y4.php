...
while ($d = mysqli_fetch_array($data)) {
    $pembayaranData[] = [
        ...
        "id_operator" => $d["id_operator"],
        "nama_santri" => $d["nama_santri"],
        "id_pembayaran" => $d["id_pembayaran"],
    ];
}

// Sorting and searching based on nama_santri, id_operator, Metode_bayar, and id_pembayaran
$sortBy = isset($_GET['sort']) ? $_GET['sort'] : 'nama_santri';
$sortedData = bubbleSort($pembayaranData, $sortBy);
...
foreach ($sortedData as $item) {
    if (
        ...
        stripos($item["Metode_bayar"], $searchTerm) !== false ||
        $item["id_operator"] == $searchTerm ||
        $item["id_pembayaran"] == $searchTerm
    ) {
        $searchResults[] = $item;
    }
}
    ...
    <div class="p-3 text-info-emphasis bg-info-subtle border border-info-subtle rounded-3">
        ...
        <form method="get" action="">
            <div class="input-group mb-3">
                <input type="text" class="form-control" placeholder="Cari Nama, Metode Bayar, ID Operator, atau ID Pembayaran" name="search" value="<?= $searchTerm ?>">
        ...
        <table class="table">
            <thead>
                <tr><br><br>
                    ...
                    <th scope="col"><a href="?sort=id_operator">ID OPERATOR</a></th>
                    <th scope="col"><a href="?sort=Metode_bayar">METODE BAYAR</a></th>
                    <th scope="col"><a href="?sort=id_pembayaran">ID PEMBAYARAN</a></th>
                </tr>
            </thead>
            <tbody>
                <?php
